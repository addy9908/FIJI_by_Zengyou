...
# ZY: change to transformation=[Translation]
# version:
#   August, 2014: v0.1
...
#   2016/12/21: #add ZY's deconvolution folder under root for easy session load
# ZY: alway create 'channels8' folder and fake 8bit folder as deconvolution folder
...
def runOneFile(fullFilePath):
	...
	if gGetNumChanFromScanImage:			
		...
		if scanimage3:
			gNumChannels = int(infoStr.split('numberOfChannelsSave=')[1].split('\r')[0])	
		...
		if scanimage4:
			gNumChannels = int(len(infoStr.split('numberOfChannelsSave=')[1].split('\r')[0])/2)
			bPrintLog('over-riding gNumChannels with: ' + str(gNumChannels), 2)
	...
	if gDoAlign and gNumChannels == 1 and copy.getNSlices()>1:
		...
		stackRegParams = 'stack_1=[%s] action_1=Align file_1=[%s] stack_2=None action_2=Ignore file_2=[] transformation=[Translation] save' %(impWin,transformationFile)
		...
		stackRegParams = 'stack_1=[%s] action_1=[Load Transformation File] file_1=[%s] stack_2=None action_2=Ignore file_2=[] transformation=[Translation]' %(origImpTitle,transformationFile)
		IJ.run('MultiStackReg', stackRegParams)		
	...
	if gDoAlign and gNumChannels == 2 and ch1Imp.getNSlices()>1 and ch2Imp.getNSlices()>1:
		...
		stackRegParams = 'stack_1=[%s] action_1=Align file_1=[%s] stack_2=None action_2=Ignore file_2=[] transformation=[Translation] save' %(alignThisWindow,transformationFile)
		IJ.run('MultiStackReg', stackRegParams)
		...
		stackRegParams = 'stack_1=[%s] action_1=[Load Transformation File] file_1=[%s] stack_2=None action_2=Ignore file_2=[] transformation=[Translation]' %(origCh1WinStr,transformationFile)
		IJ.run('MultiStackReg', stackRegParams)		
		# ch2
		bPrintLog('MultiStackReg applying alignment to:' + origCh2WinStr, 1)
		stackRegParams = 'stack_1=[%s] action_1=[Load Transformation File] file_1=[%s] stack_2=None action_2=Ignore file_2=[] transformation=[Translation]' %(origCh2WinStr,transformationFile)
		IJ.run('MultiStackReg', stackRegParams)		
...
def run():
	bPrintLog(' ', 0)
	bPrintLog('=====================================', 0)
	bPrintLog('Running bAlign_Batch_v7_1', 0)
	bPrintLog('=====================================', 0)
	...
	bPrintLog('=====================================', 0)
	bPrintLog('Done bAlign_Batch_v7_1', 0)
	bPrintLog('=====================================', 0)
        bPrintLog(' ', 0)
