...
# 7/29/2016: debug size problem, make code shorter
#V7.1: try to close original during process: use restacked copy (forget it!)
...
def selectBest(imp):
	...
	IJ.run(imp,"Set... ", "zoom=" + str(mZoom/2) +" x=0 y=0")
	...
	if nFrame == 1:
		...
		#set zoom
		IJ.run(resort_stack,"Set... ", "zoom=" + str(mZoom/2) +" x=0 y=0")
		...
		for i in range(0,m):
			...
			zMax = preview_Z(resort_stack,numRep,i)
			...
			IJ.run(m1_imp,"Set... ", "zoom=" + str(mZoom) +" x=0 y=0")
			m1_title=m1_imp.getTitle()
			#merge the previous one if i>0
			prePick = 1	if not sPick else sPick[i-1]	#use the first one as mask at beginning, then previous selected
			...
			#do the switchSlice on resort_stack: move to the initial of pick: j
			resort_stack = switchSlice(resort_stack,pickN,j)

			
			zMax.close()	#clear previous Z projection
			 #show the new preview based on new pick
...
def zyMergeView(imp1,imp2):
	...
	#set zoom
	IJ.run(imp1,"Set... ", "zoom=" +str(mZoom)+ " x=0 y=0")
	imp2.changes = 0
	imp2.close()
	return imp1
...
def switchSlice(imp,sourceNum,destNum):
...
def doAlign(imp,destFolder):
	...
	transformationFile = destFolder + '\\' + os.path.splitext(filename)[0] + '.txt' #could make the txt to subfolder here
	printLog(transformationFile,1)
...
def zyGetProperty(imp,fields): #modify it for different image source, this if for scanimage 3
...
if __name__ == '__main__': 
	printLog("===========================================================",0)
	printLog("  ********rep_stack_V7:"+time.strftime("%c")+" ***********",0)

	try:
		imp = IJ.getImage()	#for opened image
	except:
		printLog("No image found, please select one",0)
		imp = openFile()	#prompt user to open an image
	if imp:
		#auto set the zoom of montaged image
		global mZoom
		screen = IJ.getScreenSize()
		screenWidth = screen.width
		impSize = imp.width
		printLog("screenWidth is:" + str(screenWidth)+", image size is:" + str(impSize),0)
		mZoom = 100*screenWidth*1024/(2560*impSize) #100 for 1560 width screen when image is 1024X1024
		printLog("mZoom is: " + str(mZoom),0)
